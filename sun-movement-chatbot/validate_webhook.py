...
def test_rasa_directly():
    ...
    for test in TEST_MESSAGES:
        try:
            ...
            if console:
                console.print(f"\n[cyan]Testing[/cyan]: '{message}' [dim]({description})[/dim]")
            else:
            ...
            if response.status_code == 200:
                ...
                if responses:
                    if console:
                        console.print("[green]✓ Phản hồi nhận được:[/green]")
                        for idx, resp in enumerate(responses):
                            console.print(f"  {idx+1}. {resp.get('text', 'Không có nội dung')}")
                    else:
                    ...
                    if console:
                        console.print("[yellow]⚠ Không có phản hồi (mảng rỗng)[/yellow]")
                    else:
                        print("⚠ Không có phản hồi (mảng rỗng)")
                    failed_tests += 1
            else:
                if console:
                    console.print(f"[red]✗ Lỗi - Status code: {response.status_code}[/red]")
                else:
            ...
            if console:
                console.print(f"[red]✗ Lỗi khi gửi tin nhắn: {str(e)}[/red]")
            else:
...
def test_frontend_proxy():
    ...
    for test in proxy_tests:
        try:
            ...
            if console:
                console.print(f"\n[cyan]Testing via proxy[/cyan]: '{message}' [dim]({description})[/dim]")
            else:
            ...
            if response.status_code == 200:
                ...
                if responses and len(responses) > 0 and not any("không hiểu" in str(r.get("text", "")).lower() for r in responses):
                    if console:
                        console.print("[green]✓ Phản hồi proxy nhận được:[/green]")
                        for idx, resp in enumerate(responses):
                            console.print(f"  {idx+1}. {resp.get('text', 'Không có nội dung')}")
                    else:
                    ...
                    if console:
                        console.print(f"[yellow]⚠ Phản hồi mặc định (fallback): {fallback_text}[/yellow]")
                    else:
                        print(f"⚠ Phản hồi mặc định (fallback): {fallback_text}")
                    failed_tests += 1
            else:
                if console:
                    console.print(f"[red]✗ Lỗi proxy - Status code: {response.status_code}[/red]")
                else:
            ...
            if console:
                console.print(f"[red]✗ Lỗi khi gửi tin nhắn qua proxy: {str(e)}[/red]")
            else:
...
def print_summary(direct_results, proxy_results):
    ...
    if console:
        console.print(f"[bold]Kết nối trực tiếp:[/bold]")
        console.print(f"  [green]✓ Thành công[/green]: {direct_success}/{total_direct} ({direct_success_rate:.1f}%)")
        console.print(f"  [red]✗ Thất bại[/red]: {direct_fail}/{total_direct} ({100-direct_success_rate:.1f}%)")
        
        console.print(f"\n[bold]Kết nối qua proxy:[/bold]")
        console.print(f"  [green]✓ Thành công[/green]: {proxy_success}/{total_proxy} ({proxy_success_rate:.1f}%)")
        console.print(f"  [red]✗ Thất bại[/red]: {proxy_fail}/{total_proxy} ({100-proxy_success_rate:.1f}%)")
    else:
    ...
    if direct_success_rate > 80 and proxy_success_rate < 50:
        if console:
            console.print("\n[bold yellow]KẾT LUẬN:[/bold yellow] Rasa server hoạt động bình thường nhưng có vấn đề với API proxy. Kiểm tra file route.ts trong frontend.")
        else:
        ...
        if console:
            console.print("\n[bold red]KẾT LUẬN:[/bold red] Có vấn đề với cả Rasa server và API proxy. Kiểm tra lại model, dữ liệu train và kết nối.")
        else:
        ...
        if console:
            console.print("\n[bold green]KẾT LUẬN:[/bold green] Hệ thống hoạt động tốt. Vấn đề có thể nằm ở giao diện frontend hoặc ngữ cảnh đặc biệt.")
        else:
            print("\nKẾT LUẬN: Hệ thống hoạt động tốt. Vấn đề có thể nằm ở giao diện frontend hoặc ngữ cảnh đặc biệt.")
    else:
        if console:
            console.print("\n[bold yellow]KẾT LUẬN:[/bold yellow] Có một số vấn đề cần được khắc phục. Kiểm tra các intent cụ thể đang gặp vấn đề.")
        else:
...
def main():
    ...
    if not check_rasa_status():
        if console:
            console.print("\n[bold red]Không thể kết nối đến Rasa server. Vui lòng đảm bảo server đang chạy.[/bold red]")
        else:
